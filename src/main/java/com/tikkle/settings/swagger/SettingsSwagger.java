...
@Tag(name = "Settings", description = "설정 API (투자 룰 · 알림)")
public interface SettingsSwagger {

    @SecurityRequirement(name = "bearerAuth")
    @Operation(summary = "설정 전체 조회", description = "매매 방식, 7개 카테고리 잔돈 룰, 푸시 알림 on/off를 한 번에 조회합니다. 미설정 카테고리는 NONE으로 반환됩니다.")
    ...
    @SecurityRequirement(name = "bearerAuth")
    @Operation(summary = "푸시 알림 설정 변경", description = "푸시 알림 전체 on/off를 변경합니다.")
    ApiResponse<?> updateNotification(@Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
