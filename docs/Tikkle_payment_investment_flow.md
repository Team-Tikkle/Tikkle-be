...
> **타임아웃 판정은 입금 상태 조회 뒤에 한다.** 경과 시간만으로 먼저 끊으면, 폴링이 다른 작업에 밀려 지연됐을 때 **실제로 도착한 입금까지 조회 없이 타임아웃 처리**된다. 그 뒤 사용자가 재승인하면 두 번째 입금 요청이 나가 **원화가 이중 입금**되고 첫 입금분은 업비트에 잔류한다. 계좌 정보가 없어 조회 자체가 불가능한 경우에만 경과 시간으로 판단한다.

| 입금 상태 | 처리 |
| --- | --- |
| 그 외 (대기 중) | SSE `PROCESSING` 발송 후 다음 주기 대기 |
| `ACCEPTED` | 즉시 시장가 매수 실행 (§10.2). **210초가 지났더라도 입금이 도착했으면 매수한다** |
| `REJECTED` / `CANCELED` | `FAILED` 마킹 + SSE `DEPOSIT_FAILED` + FCM `DEPOSIT_FAILED` (출금된 원화 없음) |
| **210초 경과 + 입금 미도착** | **`PENDING_PURCHASE`로 복구**(재승인 가능) + SSE `TIMEOUT` + FCM `DEPOSIT_TIMEOUT` |
...
**스레드 격리 정책** — 스프링 기본 스케줄러 풀은 크기가 1이라 오래 걸리는 작업 하나가 나머지 전부를 막는다. `SchedulingConfig`에서 전용 `TaskScheduler`(풀 5, `tikkle-sched-*`)를 구성하고, 수 분이 걸리는 작업은 `@Async`로 스케줄러 스레드를 즉시 반납한다.

| 작업 | 처리 |
| --- | --- |
| `AiPortfolioScheduler` (정기 실행 + 기동 시 실행) | `@Async` — 9개 조합 × 최대 3회 재시도로 수십 분이 걸릴 수 있다 |
| `MarketTopicScheduler` (기동 시 실행) | `@Async` — 외부 RSS 호출이 기동을 막지 않도록 |
| 입금·체결 폴링 | 스케줄러 스레드에서 직접 실행 (짧고 주기가 촘촘함) |

> 이 격리가 없으면 AI 유니버스 생성이 도는 **14:00에 3초 주기 입금 폴링이 수십 분간 멈춘다.** 사용자가 결제하는 시간대이고, 그 사이 도착한 입금이 타임아웃으로 오판되어 원화 이중 입금으로 이어진다(§10.1).
