...
**승인/거절 버튼은 `status === 'PENDING'`일 때만 노출하세요.** `IN_PROGRESS`에서 승인을 요청하면 `400 PAYMENT-006 "결제 대기 상태가 아니므로 처리할 수 없습니다"`가 떨어집니다.
...
**SSE로 실제 전달에 성공한 이벤트는 서버가 FCM을 억제합니다.** 앱에서 중복 제거 로직을 넣을 필요가 없습니다.

전달에 실패했을 때만 FCM이 나가므로, "SSE도 받고 FCM도 받는" 상황은 생기지 않습니다. 반대로 앱이 이미 떠나 SSE가 닿지 않은 결과는 서버가 FCM 발송을 **시도**합니다.

> **FCM 도달 자체는 보장되지 않습니다.** `tikkle.fcm.enabled=false`이거나, 디바이스 토큰이 등록돼 있지 않거나, 토큰이 무효화된 경우 서버는 발송을 생략하고 로그만 남깁니다. FCM 자체도 best-effort 전달입니다. 따라서 **알림을 결과 전달의 유일한 경로로 삼지 마세요** — 앱 재진입 시 `GET /api/payments/in-progress`(§2)와 결제 내역 재조회로 항상 최신 상태를 확인할 수 있어야 합니다.
...
- **승인 재요청은 막혀 있습니다.** `PENDING_DEPOSIT` 상태에서 승인을 다시 요청하면 `400 PAYMENT-006`입니다. 기존 입금 요청이 살아있는데 재요청하면 2차 인증이 두 개 발송되어 원화가 이중 입금될 수 있어 의도적으로 막았습니다. 재승인은 210초 타임아웃 이후에만 가능합니다.
